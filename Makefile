...
verify-agency-kpi-history:
	...
	export EXPECTED; \
	printf '%s\n' "$$DATES" | xargs -n 1 -P "$(BUNCHING_VERIFY_PARALLEL)" sh -c 'day="$$1"; curl -fsS --connect-timeout 10 --max-time 60 "https://storage.googleapis.com/$(PROJECT_ID)-actransit-cache/stats/$${day}.json" | jq -e --argjson expected "$${EXPECTED}" '\''.agency_kpi.methodology_version == $$expected and all(.routes[]; .agency_kpi.methodology_version == $$expected and ((.agency_kpi.service_operated.scheduled_trips == 0 and .agency_kpi.on_time_performance.scheduled_timepoints == 0) or ((.agency_kpi_by_day_type | length) > 0 and all(.agency_kpi_by_day_type[]?[]?; .methodology_version == $$expected) and ([.agency_kpi_by_day_type[]?[]?.service_operated.scheduled_trips] | add // 0) == .agency_kpi.service_operated.scheduled_trips and ([.agency_kpi_by_day_type[]?[]?.on_time_performance.scheduled_timepoints] | add // 0) == .agency_kpi.on_time_performance.scheduled_timepoints)))'\'' >/dev/null || { echo "MISMATCH day $$day"; exit 1; }' sh; \
	printf '%s\n' "$$WEEKS" | xargs -n 1 -P "$(BUNCHING_VERIFY_PARALLEL)" sh -c 'week="$$1"; curl -fsS --connect-timeout 10 --max-time 60 "https://storage.googleapis.com/$(PROJECT_ID)-actransit-cache/stats/weekly/$${week}.json" | jq -e --argjson expected "$${EXPECTED}" '\''.agency_kpi.methodology_version == $$expected and all(.route_daily_service_delivered[]; .agency_kpi.methodology_version == $$expected and ((.agency_kpi.service_operated.scheduled_trips == 0 and .agency_kpi.on_time_performance.scheduled_timepoints == 0) or ((.agency_kpi_by_day_type | length) > 0 and all(.agency_kpi_by_day_type[]?[]?; .methodology_version == $$expected) and ([.agency_kpi_by_day_type[]?[]?.service_operated.scheduled_trips] | add // 0) == .agency_kpi.service_operated.scheduled_trips and ([.agency_kpi_by_day_type[]?[]?.on_time_performance.scheduled_timepoints] | add // 0) == .agency_kpi.on_time_performance.scheduled_timepoints)))'\'' >/dev/null || { echo "MISMATCH week $$week"; exit 1; }' sh; \
	printf '%s\n' "$$MONTHS" | xargs -n 1 -P "$(BUNCHING_VERIFY_PARALLEL)" sh -c 'month="$$1"; curl -fsS --connect-timeout 10 --max-time 60 "https://storage.googleapis.com/$(PROJECT_ID)-actransit-cache/stats/monthly/$${month}.json" | jq -e --argjson expected "$${EXPECTED}" '\''.agency_kpi.methodology_version == $$expected and all(.routes[]; .agency_kpi.methodology_version == $$expected and ((.agency_kpi.service_operated.scheduled_trips == 0 and .agency_kpi.on_time_performance.scheduled_timepoints == 0) or ((.agency_kpi_by_day_type | length) > 0 and all(.agency_kpi_by_day_type[]?[]?; .methodology_version == $$expected) and ([.agency_kpi_by_day_type[]?[]?.service_operated.scheduled_trips] | add // 0) == .agency_kpi.service_operated.scheduled_trips and ([.agency_kpi_by_day_type[]?[]?.on_time_performance.scheduled_timepoints] | add // 0) == .agency_kpi.on_time_performance.scheduled_timepoints)))'\'' >/dev/null || { echo "MISMATCH month $$month"; exit 1; }' sh; \
...
smoke:
	...
	bq show -q $(PROJECT_ID):actransit.api_request_hourly >/dev/null 2>&1 || { echo "FAIL: actransit.api_request_hourly missing"; exit 1; }; \
	echo "  OK"; \
